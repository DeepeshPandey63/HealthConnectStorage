...
const Room = () => {
  ...
  const handleUserJoined = useCallback(({ email, id, appointmentId }) => {
    console.log(`Email ${email} joined room with appointmentId: ${appointmentId}`);
    setRemoteSocketId(id);
    console.log(`Remote Socket ID: ${id}`);
    handleCallUser(id, appointmentId);
  }, []);
  ...
  const handleCallUser = useCallback(
    async (id, appointmentId) => {
      ...
      socket.emit("user:call", { to: id, offer, appointmentId });
    },
    [socket]
  );
  ...
  const handleIncomingCall = useCallback(
    async ({ from, offer, appointmentId }) => {
      try {
        ...
        setMyStream(stream);
        console.log(`Incoming Call from ${from} with appointmentId: ${appointmentId}`);
